...
**MCP `2026-07-28` — the lints have landed, the transport has not, and that split is deliberate.** Spec §0 and §12 carry what changed.

*Landed:* declaration ordering plus `unstable-list-order`, the SEP-2549 cache lints (`missing-cache-ttl`, `missing-cache-scope`, `public-cache-scope`) in `src/analyser/rules/cache.ts`, a `server/discover` probe that degrades silently to the legacy handshake, and `legacy-protocol` to say out loud when the cache checks were *skipped* rather than passed.

*Not landed, and blocked rather than deferred:* the stateless transport. **No shipped TypeScript SDK speaks the revision** — `@modelcontextprotocol/sdk@1.30.0` is `latest` and still declares `LATEST_PROTOCOL_VERSION = '2025-11-25'`, with no `server/discover` and no routing headers. Building it now means hand-rolling a transport against a spec document with no server to verify it against, so the decision was to wait. The lints did not have to wait because the SDK's `ResultSchema` is a Zod *loose* object: unknown keys survive parsing, so `ttlMs` and `cacheScope` are readable through the current SDK the moment a server sends them.

Publication is a publish date, not a switch-off — the servers M5 depends on are overwhelmingly still on `2025-11-25`, which is why every cache rule is gated on the protocol revision and why `legacy-protocol` exists. Dual-protocol remains a prerequisite for M5, and sits ahead of the second-provider work in [`plans/multi-provider-plan.md`](plans/multi-provider-plan.md), which was already gated on a paid calibration run.
