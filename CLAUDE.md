...
**MCP `2026-07-28` shipped on 28 July 2026, and the connector does not speak it yet.** Spec §0 and §12 carry what changed. Two consequences: the ordering fix and `unstable-list-order` have landed (§12.1's cheapest check — see the measurement decisions below), but the **dual-protocol connector has not started**, and it is a prerequisite for M5 rather than an upgrade. Publication is a publish date, not a switch-off: the servers the leaderboard depends on are overwhelmingly still on `2025-11-25`, so the connector has to probe `server/discover` and fall back to the legacy `initialize` handshake. `ttlMs`, `cacheScope` and the `server/discover` cross-check are the other free lints and are not built. That work sits ahead of the second-provider work in [`plans/multi-provider-plan.md`](plans/multi-provider-plan.md), which was already gated on a paid calibration run.
